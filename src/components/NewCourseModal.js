...
import { createCourse, updateCourse } from "../redux/actions/coursesActions";
import { connect } from "react-redux";
import { getProfile } from "../helpers/utils";
import { toast } from "react-toastify";

const NewCourseModal = ({ handleShow, courseInfo, show, ...props }) => {
  ...
  const uploadImage = (data) => {
    ...
    return fetch(
      "https://api.Cloudinary.com/v1_1/broadcaster/image/upload",
      options
    )
      .then((res) => res.json())
      .then(({ url }) => {
      ...
      if (coverImage[0] !== undefined) {
        ...
        if (courseInfo && courseInfo.id) {
          props.updateCourse({
            ...
            description: courseDescription,
            price: coursePrice,
            hasImage: coverImage[0] !== undefined ? true : courseInfo.hasImage,
          });
          clearInput();
          return handleClose();
        }
  ...
  const changeHandler = (evt) => {
    evt.target.name === "name"
      ...
      : evt.target.name === "credit"
      ? setcourseCredit(evt.target.value)
      : evt.target.name === "price"
      ? setcoursePrice(evt.target.value)
      : evt.target.name === "description"
      ? setcourseDescription(evt.target.value)
      : void 0;
  };
  ...
  useEffect(() => {
    if (courseInfo) {
      ...
      setcourseCredit(courseInfo.credit);
              ...
              <form onSubmit={SendHandler}>
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="eg: JavaScript Fundamentals"
                  />
                </div>
                <div className="form-group">
                  <label htmlFor="courseName">
                  ...
                  <input
                    ...
                    name="code"
                    onChange={changeHandler}
                    placeholder="eg: JS 101"
                  />
                </div>
                <div className="form-group">
                  <label htmlFor="courseName">
                    Course Credit<span className="required"> *</span>
                  </label>
                  <input
                    type="number"
                    className="form-control"
                ...
                <div className="">
                  {isDragAccept && (
                    <div>
                      ...
                      <div className="text-success">Release to drop here</div>
                    </div>
                  )}
                  {isDragReject && (
                    <div>
